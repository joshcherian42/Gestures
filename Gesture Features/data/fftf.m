function [X, f, freq_unf, freq_filter] = fftf(t, x, varargin)
% fftf - fft filter;
...
%% main
freq_unf=fft(x,Nfft)/N; % perform fft transform
freq_filter=filterfft(f, freq_unf, cutoff_freq, my_freqs); % filter amplitudes
%X=ifft(y2,'symmetric'); % the inverse transform. 'symmetric' is not recognized in older versions of matlab 
X=ifft(freq_filter); % inverse transform
X=X(1:N)/max(X);
ind1 = find(freq_filter(1:1+Nfft/2)); % get the nonzero elements in y2
